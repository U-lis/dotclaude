...
| URL + `--prefill` simultaneously | A positional argument matches the GitHub URL regex AND the `--prefill` flag is present | Invoke `Skill("dotclaude:init-prefill")`. Forward the URL as the `url_reference` context variable AND the prefill body as the `prefill_body` context variable. The URL is NOT fetched here; it is resolved inside `init-prefill.md` Step 2.5 (FR-9 Scenario A), which performs the `gh` fetch and prompts the user with a 4-option `AskUserQuestion` (Merge / URL Override / URL Ignore / Cancel; default = Merge per AD-4). Unlike row 1 (URL only -> `init-github-issue`), this row always treats prefill as the primary input and the URL as supplementary context unless the user explicitly chooses URL Override or Cancel. |
...
- **URL + `--prefill` simultaneously**: Both inputs are forwarded to `init-prefill` (`url_reference` and `prefill_body` respectively). The FR-9 flow is handled entirely inside `init-prefill.md` Step 2.5: the URL is fetched via `gh issue view` or `gh pr view`, then the user is prompted with a 4-option `AskUserQuestion` (Merge / URL Override / URL Ignore / Cancel). The default option is "Merge (recommended), prefill takes precedence" per DESIGN AD-4. On `gh` fetch failure, init-prefill falls back to prefill-only and preserves the URL as a plain reference in SPEC.md (per FR-9 last clause and AD-6). The difference from row 1 (URL only) is that row 1 routes to `init-github-issue` and uses the URL as the sole input, whereas row 3 keeps prefill as the primary input.
...
PLAN: {working_directory}/{doc_dir}/PHASE_{phase_id}_PLAN_{keyword}.md
TEST: {working_directory}/{doc_dir}/PHASE_{phase_id}_TEST.md
GLOBAL: {working_directory}/{doc_dir}/GLOBAL.md
...
- **PASS (recommendation: COMMIT)**: Commit the phase (all code files + updated documents)
  ...
  git add {working_directory}/{doc_dir}/PHASE_{phase_id}_PLAN_{keyword}.md
  git add {working_directory}/{doc_dir}/GLOBAL.md
  git add {working_directory}/{doc_dir}/PHASE_{phase_id}_TEST.md
